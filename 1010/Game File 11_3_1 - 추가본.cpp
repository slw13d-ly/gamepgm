...
int play_again(); // 재시작 기능을 위한 함수 선언
...
int play_again() 
{
    int choice;
    gotoxy(2, 21);
    printf("게임을 다시 시작하려면 1을, 종료하려면 0을 입력하세요: ");
    scanf("%d", &choice);

    while (choice != 1 && choice != 0) 
    {
        gotoxy(2, 22);
        printf("잘못된 입력입니다. 1(재시작) 또는 0(종료)을 입력하세요: ");
        scanf("%d", &choice);
    }
...
void game_intro() 
{
    ...
    printf("  게임이 시작되고 30초간 입력이 없으면 게임이 종료됩니다.\n");
	printf("\n");
...
void game_control(char question[][10], int n, int *count)
{
    ...
    start_time = clock();  // 시간 시작
    
    gotoxy(2, 16);
    printf("첫번째 숫자를 입력하고 Enter>");

    while (!_kbhit())  // 사용자의 입력을 기다리는 루프
    {
        current_time = clock();  // 현재 시간 갱신
        if ((current_time - start_time) / CLOCKS_PER_SEC > 30)  // 30초가 넘었는지 확인
        {
            gotoxy(2, 20);
            printf("30초 동안 입력이 없어 게임을 종료합니다.\n");
            exit(0);
        }
    }

    scanf("%d", &user1);  // 첫 번째 숫자 입력
    start_time = clock();  // 시간 다시 시작
    ...
    scanf("%d", &user2);  // 두 번째 숫자 입력
    row2 = user2 / n;
    col2 = user2 % n;

    // 이후의 게임 로직은 기존 코드와 동일
